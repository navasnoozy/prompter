...
- Quick Capture (`Shift + Command + P` by default, rebindable from Settings) first reads selected text directly through macOS Accessibility. For controls that do not expose selection text, it snapshots the clipboard, checks the focused control, frontmost process, and pasteboard change count, and attempts restoration only while the observed change count still matches its capture transaction. It never sends the text automatically.
